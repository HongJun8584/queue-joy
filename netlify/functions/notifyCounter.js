// netlify/functions/notifyCounter.optimized.js
// Optimized version based on your checklist:
// - Preload series from Firebase (query) to avoid per-ticket reads
// - Build messages in-memory (no fetch inside loop)
// - Batch Telegram sends with Promise.allSettled
// - Avoid repeated disk I/O: only write fallback /tmp once per invocation
// - When using Redis, batch Redis writes where possible
// - Reduce repeated normalization by caching values early
...
async function saveSeriesStats(series, stats, store, storeDirtyFlag) {
  if (useRedis) {
    ...
    storeDirtyFlag.dirty = true; // signal to save once at the end
  }
}
...
async function fetchQueueForSeries(series) {
  try {
    // RTDB expects quoted orderBy param
...
exports.handler = async function (event) {
  ...
  if (!rawRecipients.length) {
    // try to fetch by series first (fast if series is indexed on RTDB)
    if (calledSeries) {
      preloadQueue = await fetchQueueForSeries(calledSeries);
    }
    // if no results (or series empty), fallback to fetching everything
  ...
  for (const [key, item] of dedupe.entries()) {
    ...
    if (ageMs > MAX_AGE_MS && !isMatch && ticket.ticketId) {
      ...
      // persist ticket update (defer file write, or queue redis set)
      if (useRedis) {
    ...
    if (isMatch) {
      ...
      if (ticket.ticketId) {
        ...
        // push service event (fire-and-forget but non-blocking)
        pushServiceEvent({ ticketId: ticket.ticketId, requestedAt: ticket.createdAtMs || createdMs, servedAt: now, serviceMs, counter: counterName || null, series: ticket.series || calledSeries });
      } else {
        const serviceMs = Math.max(0, now - (ticket.createdAtMs || createdMs || now));
        pushServiceEvent({ ticketId: null, requestedAt: ticket.createdAtMs || createdMs, servedAt: now, serviceMs, counter: counterName || null, series: ticket.series || calledSeries });
      }
  ...
  // ---------- Send Telegram messages in parallel (but consider API rate limits) ----------
  if (telegramPrepared.length) {
    // Optionally chunk if you expect huge volumes to avoid rate limits. Here we send them all in parallel.
